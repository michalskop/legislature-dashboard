...
import { getAllMpProfiles, getAllPartyProfiles, getGovernmentAxisPlacement } from "@/lib/data";
...
export default async function CityHomePage({ params }: Props) {
  ...
  const [allMps, parties, govAxis] = await Promise.all([
    ...
    <div className="space-y-10">
      ...
      <PageBlockRenderer
        ...
        ctx={{
          ...
          // DIVERGENCE.md §8 (a)) — so the coalition/opposition label text
          // goes wherever that axis renders, not always on x, and its word
          // order follows government_sign (round 4 fix, §8) so the word after
          // the arrows matches whichever end government is actually on.
          chartLabels: (() => {
            const govLabel =
