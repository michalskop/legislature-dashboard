...
export async function generateMetadata({ params }: Props) {
  ...
  return buildGlobalMetadata({ lang, path: "/about", title: content.pageTitle });
}
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 py-8">
        <div className="max-w-2xl space-y-10">
          ...
          <div className="pt-4 border-t border-border">
            <Link href={globalBasePath(lang) || "/"} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
              {content.backLabel}
            </Link>
          </div>
        </div>
